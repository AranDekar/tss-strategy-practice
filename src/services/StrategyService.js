  ...
  */
  static async getYearlyReport(strategy) {
    return StrategyReportSummary.where({ strategy })
      .select('instrument year total maxProfit maxLoss').exec();
  }
  ...
  */
  static async getMonthlyReport(strategy, instrument) {
    ...
    const data = await StrategyReportSummary.where({ strategy, instrument })
      .select('instrument year monthly.month monthly.total monthly.maxProfit  monthly.maxLoss').exec();
    for (const item of data) {
      const { year } = item;
      for (const monthly of item.monthly) {
        const {
          month, total, maxProfit, maxLoss,
        } = monthly;
        result.push({
          strategy, instrument, year, month, total, maxProfit, maxLoss,
        });
      }
    }
    return result;
